// Agent toaB in project trip_planner

/* Initial beliefs and rules */
holiday_package("HP2","Paris", "Accomodation", "Guide", "Food", "Flight").
my_budget("Accomodation", 600).
my_budget("Flight", 300).
my_budget("Guide", 250).
my_budget("Food", 100).
reputation(0.8).
...
+!create_auction_artifacts: holiday_package(HPId, Destination, Service1, Service2, Service3, Service4)
	<-
...
+!advertise_packages
	<- 
	?holiday_package(HPId, Destination, Service1, Service2, Service3, Service4);
...
+!go <-  .my_name(Me);
         ...
         makeArtifact(ora4mas, "ora4mas.nopl.OrgBoard", ["src/org/org_hp_B.xml"], OrgArtId)[wid(WOrg)];
      	focus(OrgArtId);
